...
function exportRowLines(row,siblings){
  const out=[];
  const isC=row.kind==='commander';
  const tot=rowTotal(row);
  const tier=row._isCustom?`Custom T${row.customRank||2} ${row.tier}`:row.tier;
  const count=isC?'':` ×${parseInt(row.warriors)||1}`;
  out.push(`- ${row.unit} (${tier}${count}) — ${tot} pts`);
  ...
  const inh=getInherent(row.unitData?.full_profile||'');
