import { BW_DATA, EQUIP, CG_UPGRADES } from '../data/game-data.js?v=20260517b-lowlander-bill-regulars';
...
function abilityChip(name,rule){
  ...
  return `<span class="abi-chip" tabindex="0" onclick="toggleAbilityChip(event,this)"><span class="abi-chip-name">${esc(name)}</span><span class="abi-chip-tip">${tipLabel?`<span class="abi-chip-tip-label">${esc(tipLabel)}</span>`:''}${esc(r)}</span></span>`;
}
...
// EXPORT
// ═══════════════════════════════════════════════════════════
// Plain-text/markdown summary of the current list
function buildExportText(){
  const lines=[];
  const cap=state.ptsCap, spent=totalPts();
  lines.push("# Barons' War List");
  lines.push(`Points: ${spent} / ${cap}`);
  if(!state.factions.length){lines.push('','(empty list)');return lines.join('\n');}
  if(isCombined()){
    lines.push(`Combined Retinue · ${state.factions.length} retinues · Liege: ${facLabel(liegeFaction())}`);
  } else {
    lines.push(`Retinue: ${facLabel(state.factions[0])}`);
  }
  if(state.mercenaryCompany)lines.push(`Mercenary Company: ${state.mercenaryCompany}`);
  lines.push('');
  ...
  for(const fid of state.factions){
    ...
    lines.push(`## ${facLabel(fid)}${role} — ${sub} pts`);
    if(!rows.length){lines.push('  (no units)','');continue;}
    const cmds=rows.filter(r=>r.kind==='commander');
    const wars=rows.filter(r=>r.kind!=='commander');
    if(cmds.length){
      lines.push('### Commanders');
      for(const c of cmds)lines.push(...exportRowLines(c,rows));
    }
    if(wars.length){
      lines.push('### Warriors');
      for(const w of wars)lines.push(...exportRowLines(w,rows));
    }
    lines.push('');
  }
  return lines.join('\n');
...
function exportRowLines(row,siblings){
  const out=[];
  ...
  out.push(`- ${row.unit} (${tier} ×${models}) — ${tot} pts`);
  const st=calcStats(row);
  out.push(`    Stats: M ${st.move} · A ${st.attack} · D ${st.defence} · Sh ${st.shield} · Mo ${st.morale} · ${st.actions} Actions`);
  ...
  // Named characters source inherent abilities from BW_DATA.dramatis, not unitData.
  ...
  if(inh.length)out.push(`    Inherent: ${inh.join(', ')}`);
  const sa=(row.selAbilities||[]).map(a=>a.cost?`${a.name} (${a.cost} pts)`:a.name);
  if(sa.length)out.push(`    Abilities: ${sa.join(', ')}`);
  const cg=(row.selCGUpgrades||[]);
  if(cg.length)out.push(`    Command Group upgrades: ${cg.join(', ')}`);
  if(isC&&row.commandGroupRowId){
    const grp=siblings.find(r=>r.id===row.commandGroupRowId);
    if(grp)out.push(`    Command Group: ${grp.unit} (${grp.tier} ×${grp.warriors||1})`);
  }
  return out;
}
...
function exportPrintView(){
  ...
  if(!w){alert('Pop-up blocked. Allow pop-ups for this site to use Print View.');return;}
  w.document.open();w.document.write(html);w.document.close();
  // Defer print so the window can lay out
...
function exportDownloadJSON(){
  ...
  const data={schema:3,factions:[...state.factions],ptsCap:state.ptsCap,mercenaryCompany:state.mercenaryCompany,list:stripped,exportedAt:new Date().toISOString()};
  const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
  const url=URL.createObjectURL(blob);
  const ts=new Date().toISOString().replace(/[:.]/g,'-').slice(0,19);
  const name=state.factions.length?(isCombined()?'combined':facLabel(state.factions[0]).toLowerCase().replace(/\s+/g,'-')):'empty';
  const a=document.createElement('a');
  a.href=url;a.download=`bw-list-${name}-${ts}.json`;
  document.body.appendChild(a);a.click();
  setTimeout(()=>{URL.revokeObjectURL(url);a.remove();},100);
}

function openExportModal(){
  exportCopyText(); // pre-fill the textarea
